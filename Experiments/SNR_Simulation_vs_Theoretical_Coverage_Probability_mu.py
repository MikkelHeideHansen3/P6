...
n_iter = 1000   # Increase to 10000 for higher accuracy
...
def snr_coverage_theory(tau_value, lambda_, rS, rE):
    # Geometry for this lambda_
    R_min = rS - rE
    R_max = np.sqrt(rS**2 - rE**2)
    area_cap = 2 * np.pi * rS * (rS - rE)

    # Probability of at least one visible satellite
...
plt.xlabel(r"SNR threshold $\tau$ [dB]", fontsize=12)
plt.ylabel(r"$P(\mathrm{SNR} > \tau)$", fontsize=12)
plt.title(r"Effect of Satellite Density $\mu$ on SNR Coverage")
plt.grid(True, linestyle='--', alpha=0.5)
plt.legend(fontsize=9)
plt.tight_layout()

#plt.savefig("PPP_SNR_vs_mu_corrected_theory.pdf")
plt.show()